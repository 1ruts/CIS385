...
function details() {
    ...
    var x = "<table style='width:75%;'>" +
     "<tr><th colspan='2'>COOKIE LISTENING</th></tr>" + // no need to declared x again if its already declared in line #62
     "<tr><th>Cookie Name</th>" +                      // no need to declared x again if its already declared in line #62
     "<th>Cookie Value</th></tr>";                     // no need to declared x again if its already declared in line #62
    if (document.cookie.length != 0)
        for (var i = 0; i < cookie_array.length; i++)
        {
            var B = cookie_array[i].split("=");
            x += "<tr><td style='width:50%;'>" + B[0] + "</td>" + 
            "<td style='width:10;'>" + B[1] + "</td></tr>";  // Same here x was declared in line # 70
        }
    x += "</table>";
